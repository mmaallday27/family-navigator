...
export function buildLookAhead(state: FamilyState, now: Date = new Date()): Horizon[] {
  ...
  // The month after 18: SSI adult redetermination.
  if (inFuture(b18)) {
  ...
  const buckets: { id: string; label: string; sub: string; min: number; max: number }[] = [
    ...
    { id: '5y', label: 'The next 5 years', sub: 'Mapped, so nothing surprises you', min: 365, max: 1826 },
  ]
...
export function buildTimeline(state: FamilyState, now: Date = new Date()): TimelineEvent[] {
  ...
  if (!Number.isNaN(birth.getTime())) {
    ...
    const diagAge = state.child.diagnosedAt.match(/\d+/)
    if (diagAge) {
      const d = new Date(birth)
      d.setFullYear(birth.getFullYear() + Number(diagAge[0]))
      events.push({
        id: 't-diagnosis',
